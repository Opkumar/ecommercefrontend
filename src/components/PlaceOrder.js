...
function PlaceOrder() {
  ...
  useEffect(() => {
    const fetchCartAndProducts = async () => {
      try {
        const userId = localStorage.getItem('userId');
        const cartResponse = await fetch('http://localhost:4000/cart/get-cart', {
          method: 'POST',
        ...
        const productResponse = await fetch('http://localhost:4000/get-product');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const orderResponse = await fetch('http://localhost:4000/order/place', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          userId,
      ...
      const clearCartResponse = await fetch('http://localhost:4000/cart/clear', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId }),
      });
